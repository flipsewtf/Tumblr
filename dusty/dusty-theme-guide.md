...
Dusty is a responsive Tumblr theme with two sidebars, a main navigation sidebar, and an optional extra sidebar for the Update Tab and Featured Tags. Optional dark mode, search bar, featured tags, update tab, Tumblr controls, post type indicator, and truncated post tags are included. Full support of NPF posts.

Options like custom post width, font size and font-families, layout gap, photoset gutter, post information, sidebar width, sidebar image size and shape, and custom navigation labels. Optional featured tags and update tab sections, as well as choices between sharp or rounded corners.
...
Dusty is a responsive theme, so depending on your screen size, the customization window might take up a lot of the space resulting in the theme to look different. Also, the Tumblr preview window rarely displays the exact product, so before messaging me please do the following:
...
5. Click **Edit HTML**, delete all existing code, then paste in Dusty's code.
...
Choose between `Nav | Extra` or `Extra | Nav`.

`Nav | Extra` places the navigation sidebar on the left and the extra sidebar on the right.

`Extra | Nav` places the extra sidebar on the left and the navigation sidebar on the right.

## Layout Gap

Choose the gap between the sidebars and post column.
...
Post info involves the dates, note count, and interactives like reblog and like buttons.
...
Choose between `40px`, `50px`, `60px`, `70px`, `80px`, `90px`, or `100px`.
...
Choose how the Tumblr controls are displayed.

| Option        | Description                             |
| ------------- | --------------------------------------- |
| Display       | displays Tumblr's controls normally     |
| Behind Toggle | hides Tumblr's controls behind a toggle |
...
The Update Tab is positioned in the extra sidebar.

To customize the Update Tab, see **Text fields: Update Tab**.
...
Toggle this on to display an icon indicating the type of each post.

## Truncated Post Tags

Toggle this on to display only the first 3 tags, with a **see all** link to reveal the remaining tags.
...
Accent Font 1 is used for various interface elements, including tooltips, main navigation, post information and post tags.
...
Accent Font 2 concerns the search field, update tab title, update tab activities, audio time display, post date year, and the Featured Tags number count.
...
It can contain HTML like links, ~~strikethrough~~, **bold**, underline, and italic text.
...
| Featured Tags Title | `Featured Tags` | The title displayed above the Featured Tags section |
...
| Text Field      | Default                   | Description                                      |
| --------------- | ------------------------- | ------------------------------------------------ |
| Update Title    | `Updates`                 | The title displayed at the top of the Update Tab |
| Update Text     | `This is an update text.` | The text displayed below the Update Tab title    |
...
| Activity 4 Icon | `gamepad-2`               | The icon for Activity 4                          |
...
You're able to change the 4 activity icons in the Update Tab. For this theme, I'm utilizing **Lucide Icons**.
